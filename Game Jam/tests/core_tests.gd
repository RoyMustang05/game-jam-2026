...
func _controller_checks() -> void:
	...
	_check(actor.animation_state == "double_jump" and actor.effects.particles.size() >= 12, "extra jump has distinct temporal burst and pose")
	await _actor_frames(1, {"axis": 0.0, "jump_held": true})
	await _actor_frames(1, {"axis": 0.0, "jump": true, "jump_held": true})
	...
	var effects: Array = actor.effects.particles.duplicate(true)
	...
	_check(actor.animation_state == "focus" and actor.effects.particles == effects, "air focus has distinct pose and frozen effect positions")
	...
	_check(actor.effects.trail.size() > 0 and String(actor.effects.trail.back().pose) == "air_dash", "dash leaves an afterimage from its first frame")
	var ghost_frame: int = int(actor.effects.trail.back().frame)
	_check(ghost_frame == int(actor._motion_clock * 36.0) % 8, "afterimage records the current animation frame, not the previous one")
