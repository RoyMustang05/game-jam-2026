...
func _run() -> void:
	...
	for i in range(4):
		...
		room.room_size = Vector2(d.width,d.height)
		room.boss = d.boss
		room.arena = d.arena
		room.par_time = d.par_time
		room.retry_budget = d.retry_budget
		room.hint = d.hint
		room.echo = d.echo
		...
		for s: Dictionary in d.crushers: _add(room,"Crusher",s)
		...
		room.free()
	print("Saved four encounter levels and reusable assets.")
	quit()
...
func _add(room: Node2D, kind: String, s: Dictionary) -> void:
	...
	for prop: String in ["speed","phase","bounceable","bounce","open_ratio","lunge"]:
